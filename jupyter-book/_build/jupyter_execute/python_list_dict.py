...
# 예를 들어, 인덱싱과 인덱스를 사용항 항목 지정이 리스트와 관련되어 가장 많이 사용되는데
...
# - `extend()`: $O(1)$
# - 리스트 연결: $O(k)$
#     - $k$는 `__add__()`를 호출하는 리스트의 길이.
#     즉, `aList + [4, 5]`에서 `aList`.
# 
...
# 
# - `range` 객체를 사용할 때 실행시간이 거의 변하지 않는다.
...
# 
# **참고**: 항목 확인과 삭제, 항목 여부 확인 알고리즘의 최악 시간복잡도는 $O(n)$이지만
...
# **참고**: 리스트, 사전, 집합, 덱(deque)의 시간복잡도에 대한 최신 정보는 
