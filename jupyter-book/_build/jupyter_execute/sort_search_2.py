...
# 
# 아래 내용을 
# [(구글 코랩) 순차 정렬, 선택 정렬, 합병 정렬, 퀵 정렬](https://colab.research.google.com/github/codingalzi/algopy/blob/master/jupyter-book/sort_search_2.ipynb)에서 
# 직접 실행할 수 있다.
...
# **버블 정렬**(bubble sort)은 연속된 두 수를 비교하여 
# 필요한 경우 작은 값은 왼쪽으로, 큰 값은 오른쪽으로 자리를 바꾸는 일을 
# 리스트 끝까지 반복하는 **패스**(pass) 과정을 리스트가 정렬될 때까지 반복하는 알고리즘이다. 
# 
...
# 하지만 파이썬은 **다중할당**(multiple assignment)을 이용하여 중간저장소 없이
...
# **선택 정렬**(selection sort)은 버블정렬의 단점인 자리교환 횟수를 패스당 최대 한 번만
# 수행하도록 개선한 기법이다. 
...
# 분할과 정복 기법을 이용하는 두 가지 정렬 알고리즘을 설명할 것이며,
# 분할과 정복 기법을 사용하기에 두 알고리즘 모두 재귀를 적용하여 구현한다.
# 
# **합병 정렬**(merge sort)에 사용되는
# 분할과 정복은 다음과 같다.
# 
# - 분할: 리스트를 반복적으로 이등분해서 생성된 모든 리스트의 길이가 1이되도록 한다. 
# - 정복: 길이가 1인 리스트는 이미 정렬되어 있으므로, 그런 리스트 두 개를 합쳐서 새로운 정렬 리스트를 만들며,
#     이 과정을 반복해서 최종적으로 원래 리스트에 포함된 항목을 정렬시킨 새로운 리스트를 얻게 된다.
...
# 분할 알고리즘의 시간복잡도를 $O(n \log n)$으로 만들 수 있다(실습 문제 참조). 
# 
# 하지만 슬라이싱 대신 구간을 사용하여 분할 과정을 추가 메모리 없이 진행하면
...
# **퀵 정렬**(merge sort) 또한 분할과 정복 기법을 활용한다.
...
# 무엇보다도 추가 메모리를 전혀 사용하지 않는다는 측면에서 합병정렬에 비해 
...
# 오론쪽 맨 끝, 또는 중앙에 위치한 값 등도 사용된다.
# 하지만 성능 차이는 기본적으로 없지만,
# 입력 사례에 따라 성능차이가 발생할 수는 있다. 
...
# 피벗을 기준으로 분할할 때 이상적인 경우 기존의 리스트를 거의 이등분한다.
...
# 1. `mergesort()` 함수에 사용된 알고리즘을 수정하여 
#     재귀에 사용된 슬라이싱 대신 구간 지정방식을 사용하는 재귀 합병 알고리즘을 구현하라.
# 
#     힌트: 합병을 담당하는 함수를 따로 정의해서 분할 재귀함수에 활용한다.
# 
# 1. 정수 500개를 무작위로 생성한 후 소개된 네 개의 정렬 알고리즘의 
#     실행시간을 평가하는 모의실험을 수행하라.
# 
# 1. 버블정렬 알고리즘을 패스별로 최댓값을 오른편으로 보내는 것과
#     최솟값을 왼편으로 보내는 과정을 번갈아 실행하도록 수정하라.
# 
# 1. 퀵정렬 알고리즘의 피벗 선택을 달리하는 알고리즘을 구현하라.
