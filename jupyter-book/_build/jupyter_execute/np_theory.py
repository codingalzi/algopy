...
#     * 정지문제(Halting problem) 처럼 지수승 이상의 만큼의 출력을 요구하진 않지만 
...
# 문제 A는 문제 B로 **다항시간 다-일 축소변환가능**(polynomial time many-one reducible),
...
# 최소 NP-완전 만큼 다루기 어려운 문제를 가리킨다.

# **P = NP ?**

# 따라서 하나의 NP-완전 문제가 P에 속한다는 것이 입증되면
# P = NP가 된다.
# 하지만 **P-NP 문제**로 알려진 P = NP의 진위 여부는 아직 답을 모르며,
# 100만 달러의 상금이 걸린 
# [밀레니엄 문제](https://ko.wikipedia.org/wiki/%EB%B0%80%EB%A0%88%EB%8B%88%EC%97%84_%EB%AC%B8%EC%A0%9C) 중에 하나다.
# 
