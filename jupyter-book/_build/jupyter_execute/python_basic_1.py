...
# 컴퓨터 과학의 기본 핵심은 **문제해결을 위한 컴퓨터 활용**이다. 
# 이 장에서 컴퓨터 과학과 관련해서 다룰 사항은 다음 두 가지이다.
# 
# - 알고리즘과 자료구조를 배우는 이유
...
# 컴퓨터 과학자는 주어진 문제를 해결하는 **알고리즘**을 개발한다.
...
# 해결이 가능한 문제를 **계산가능한(computable)** 문제라고 말한다.
...
# 변수와 연관된 값(value)을 표현하는 **표현식(expression)**은 등호 기호 오른편에 위치한다. 
# 변수가 선언되면 해당 변수는 연관된 표현식이 나타내는 값을 **참조(reference)**하도록 
# 파이썬 해석기가 메모리를 조작한다.
...
# 특정 자료형 데이터와만 함께 사용되는 함수를 **메서드(method)**라 한다.
...
# `int` 클래스가 제공하는 함수 `**add**()` 메서드를 인자 `21`과 함께 호출하여
# 실행됨을 보여준다. 기호 `+`는 `**add**()` 메서드 대신 사용되는 중위 연산자에 불과하며,
